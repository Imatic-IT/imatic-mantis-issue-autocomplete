...
export class IssueRenderer {
    ...
    public renderIssues(issues: issueModel[]): void {
        ...
        const template = html`
            ${issues.length > 0
                    ? issues.map(issue => {
                        const currentIssue = currentIssueId && currentIssueId === parseInt(issue.id, 10).toString();
                        return html`
                            <li class="${CSS_CLASSES.issuesItem}">
                                ...
                                <a href="view.php?id=${issue.id}" target="_blank"
                                   class="${currentIssue ? CSS_CLASSES.currentIssue : ''}">
                                    ${issue.id}
                                </a>
                    ...
                    : html`
                        <li class="${CSS_CLASSES.noIssuesItem}">${this.settings.noIssueFoundMessage}</li>`}
        `;
