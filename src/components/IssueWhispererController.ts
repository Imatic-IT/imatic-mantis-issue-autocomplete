...
export class IssueWhispererController {
    ...
    private setupInputListeners(): void {
        ...
        inputNames.forEach((name: string) => {
            const inputElements: NodeListOf<HTMLInputElement> = document.querySelectorAll(`input[name="${name}"]`) as NodeListOf<HTMLInputElement>

            inputElements.forEach((inputElement: HTMLInputElement) => {
                inputElement.addEventListener('focus', () => this.setActiveInputField(inputElement));
            })
        });
    }
    ...
    private handleClick(event: MouseEvent): void {
        ...
        if (target.classList.contains(CSS_CLASSES.issueButton) && target.hasAttribute('data-id')) {
    ...
    private handleKeyboardNavigation(event: KeyboardEvent) {
        ...
        this.activeHighlightedIssue = document.querySelector(`.${CSS_CLASSES.issueSelectedHighlighted}`) as HTMLLIElement;
        ...
        const moveHighlight = (newElement: HTMLElement) => {
            ...
            this.activeHighlightedIssue.scrollIntoView({behavior: 'auto', block: 'nearest'});
        }

        if (!this.activeHighlightedIssue) {
            if (['ArrowUp', 'ArrowDown'].includes(event.key)) {
                event.preventDefault();
                const firstChild = this.issueListElement.firstElementChild as HTMLLIElement;
            ...
            if (event.key === 'Enter') {
                ...
                event.preventDefault();
                const issueIdButton = this.activeHighlightedIssue.querySelector('.issue-button') as HTMLButtonElement;
